...
В MMBasic это формально работает, но **в современном MMBasic-стиле так не пишут**. Канонический бесконечный цикл — через `DO ... LOOP`:
...
`GOTO` и метки (`label:`) поддерживаются для совместимости с старым кодом, но в коммьюнити-стайл-гайде на PicoCalc прямо сказано: **«No GOTO statements»**, всё ветвление — через `IF/ENDIF`, `FOR/NEXT`, `DO/LOOP`. Если в чужом коде увидишь `GOTO` — это, как правило, сигнал, что код был портирован с line-numbered BASIC и его можно переписать чище.
...
```basic
SELECT CASE age
  CASE 0 TO 12:    PRINT "kid"
  CASE 13 TO 17:   PRINT "teen"
  CASE IS >= 18:   PRINT "adult"
END SELECT
```
...
```basic
' guess.bas - simple guessing game
' 2026-05, v0.1
...
PRINT "I picked a number from 1 to 100. Guess!"
...
Запусти, поиграй. Тот же пример расширим в [Уроке 02](02-data-and-control.md), когда углубимся в типы и управляющие конструкции.

## Упражнение

1. Перепиши `guess.bas` так, чтобы программа:
   - перед стартом спросила имя игрока (`name$`) и потом обращалась к нему по имени в выводе;
...
> Все строки, попадающие в `PRINT`/`INPUT`, оставляй на ASCII (английском). На LCD PicoCalc встроенный шрифт не содержит кириллицу — русский в коде превратится в кашу из псевдографики. В комментариях кириллица работает нормально. Подробнее в [главной странице курса](index.md#как-пользоваться).
