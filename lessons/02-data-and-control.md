...
> 📌 В коммьюнити-стайл-гайде на PicoCalc по умолчанию рекомендуют `OPTION BASE 1` — это согласуется с тем, что строковые функции (`MID$`, `INSTR`) тоже 1-based. Этот курс намеренно остаётся нейтральным к выбору базы (важно понимать, что есть выбор), но в реальном проекте лучше зафиксировать одно соглашение и держать его.
...
FOR i = 0 TO 4
  total = total + nums(i)
NEXT i
PRINT "Sum: "; total        ' 100 (0+10+20+30+40)
```
...
> 📌 Стиль-гайд коммьюнити рекомендует **избегать `:` для склейки нескольких команд в одной строке** — каждая инструкция на своей строке читается яснее. Однострочный `IF cond THEN action` (без `:`) — нормально; `IF a THEN x : y` — лучше превратить в многострочный `IF/ENDIF`. Вот эта же проверка в каноничном виде:
>
> ```basic
> IF score > best THEN
>   best = score
...
```basic
' guess2.bas - guessing game with name, rematches and win counter
' 2026-05, v0.1
...
PRINT "Hi, " + player$ + "! I'll pick numbers from 1 to 100."
  ...
  DO
    ...
    SELECT CASE guess
      ...
      CASE IS > secret
        PRINT "Lower."
      CASE ELSE
        PRINT "Got it! Tries: "; tries
        wins = wins + 1
        EXIT DO
    END SELECT
  LOOP

  PRINT player$ + ", wins this session: "; wins
  INPUT "One more round? (Y/N) ", answer$
LOOP WHILE UCASE$(answer$) = "Y"

PRINT "See you, " + player$ + "!"
```
...
- **`SELECT CASE`** с `IS <`, `IS >` — чище, чем три IF.
- **`UCASE$()`** — сравниваем с `"Y"` независимо от регистра, чтобы и `y`, и `Y` работали.
- **`LOOP WHILE`** в конце — повторяем, пока ответ — `Y`.

> ⚠️ **AUTOSAVE-нюанс.** Программу с `INPUT "..."` нельзя удобно загрузить через `AUTOSAVE` по serial: REPL воспримет `INPUT` как интерактивный prompt прямо в момент загрузки и зависнет. Варианты для переноса с компа: набрать в `EDIT` руками, либо передать через `XMODEM RECEIVE` (`F11` в REPL — но на PicoCalc этой клавиши нет, см. RC23 keymap; набирай команду полностью вручную), либо записать на SD-карточку с компа и `LOAD "B:/lessons/guess2.bas"`. Подробнее в уроке 07 (Файлы и SD).
...
2. Доработай:
   - **Сложность.** Перед началом сессии спроси, какой диапазон чисел использовать (`E` — easy: 1–50, `N` — normal: 1–100, `H` — hard: 1–500). Реализуй через `SELECT CASE` по введённой букве. Подсказка: вынеси верхнюю границу в `CONST` или в обычную переменную `max_num%` — чтобы в коде не было «магических» 50, 100, 500.
   - **Лимит попыток.** В режиме `Hard` дай только 10 попыток. Если не угадал — программа сообщает ответ и засчитывает поражение.
