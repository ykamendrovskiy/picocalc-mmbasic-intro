...
!!! note "Код на английском, комментарии на русском"
    Встроенные шрифты PicoMite (включая RC23 на PicoCalc) не содержат кириллических глифов — поэтому всё, что попадает в `PRINT` / `INPUT` / `TEXT` и рендерится на LCD, в коде уроков на английском. В **комментариях** кириллица работает свободно — токенизатор их игнорирует, на экран они не попадают.

    Если хочется русский на самом экране — это решаемо через свой CP1251-шрифт (`DefineFont`) или предотрисованный BMP, но это отдельная тема за рамками вводного курса. Подробнее — в `references/picocalc/russian_locale.md` репозитория.
